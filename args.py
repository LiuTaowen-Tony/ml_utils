...
def DataClassArg(
    ...
    **kwargs,
    ...
    """Argument helper enabling a concise syntax to create dataclass fields for parsing with `DataClassArgumentParser`.

    Example comparing the use of `DataClassArg` and `dataclasses.field`:
    ```
    @dataclass
    class Args:
        regular_arg: str = dataclasses.field(default="Huggingface", metadata={"aliases": ["--example", "-e"], "help": "This syntax could be better!"})
        DataClass_arg: str = DataClassArg(default="Huggingface", aliases=["--example", "-e"], help="What a nice syntax!")
    ```
...
class DataClassArgumentParser(ArgumentParser):
    ...
    def _parse_dataclass_field(parser: ArgumentParser, field: dataclasses.Field):
        ...
        # string is a valid attribute name. DataClass_argparser should do the same.
    ...
    def parse_args_into_dataclasses(
        ...
        if return_remaining_strings:
            ...
            if remaining_args:
                raise ValueError(f"Some specified arguments are not used by the DataClassArgumentParser: {remaining_args}")
    ...
    def parse_dict(self, args: Dict[str, Any], allow_extra_keys: bool = False) -> Tuple[DataClass, ...]:
        ...
        if not allow_extra_keys and unused_keys:
            raise ValueError(f"Some keys are not used by the DataClassArgumentParser: {sorted(unused_keys)}")
